...
const { User, Event, PlayerEvent, Report } = require("./models");
...
app.post("/create-player", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "User may already exist or Internal server error" });
  }
});
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const joinedEvents = await PlayerEvent.findAll({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      const { id } = req.params;
      const playerEvent = await PlayerEvent.findOne({
        where: { playerId: req.user.id, eventId: id },
      });
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Calculate total sessions (events)
      ...
      events.forEach((event) => {
        const participantCount = event.PlayerEvents?.length || 0;
        if (participantCount > maxParticipants) {
          maxParticipants = participantCount;
          mostPopularEventId = event.id;
        }
      });

      // Calculate participation rate
      const totalPossibleParticipants = events.reduce(
        (sum, event) => sum + event.team_limit,
        0
      );
      const actualParticipants = events.reduce(
        (sum, event) => sum + (event.PlayerEvents?.length || 0),
        0
      );
      const participationRate =
        totalPossibleParticipants > 0
          ? (actualParticipants / totalPossibleParticipants) * 100
          : 0;
      ...
      const report = await Report.create({
        ...
        endDate,
        totalSessions,
        mostPopularEvent: mostPopularEventId,
        participationRate,
      });

      res.redirect("/reports");
    } catch (error) {
      console.error("Detailed error in generate-report:", error); // Detailed error log
      res.status(500).json({
